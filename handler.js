...
module.exports = async (client, m) => {
   try {
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', '🚩 Waktu Bot Habis,Bot akan keluar terima kasih.', null, {
      ...
      if (users && (new Date * 1) >= users.expired && users.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', '🚩 Premium Mu Sudah Habis.Beli Layanan Premium Kembali.')).then(async () => {
            users.premium = false
            users.expired = 0
         })
      }
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && commands.includes((body.split` ` [0]).toLowerCase())) {
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'confess', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 Fitur Via Chat Pribadi Adalah Premium.Gunakan Premium Dan Dapatkan Lebih Banyak Limit *${prefixes[0]}owner*`, m, {
               largeThumb: true,
            ...
            if (cmd.restrict && !isOwner && text && new RegExp('\\b' + global.db.setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `🚩 Kamu Melanggar Dan Telah Terbanned Silakan Hubungi Owner`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               return client.reply(m.chat, `🚩 Kamu Sudah Kehabisan Limit.Upgrade ke premium dengan perintah *${prefixes[0]}premium*`, m).then(() => users.premium = false)
               continue
            }
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, Func.texted('bold', `🚩 Limit Kamu Habis Untuk Menggunakan Fitur Ini.`), m)
                  continue
               }
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 Menggunakan Bot Di Private Chat Merupakan Fitur Premium.\n\nIngin Premium Hubungi *${prefixes[0]}owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg'),
               url: setting.link
            }).then(() => chats.lastchat = new Date() * 1)
